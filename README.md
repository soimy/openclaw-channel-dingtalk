...
> ```
>
> 这意味着该能力对许多未开通企业认证或未购买相关能力的用户并不可用，可视为带 paywall 的平台限制。此时：
>
...
> - 这条群文件兜底链路在部分企业环境下可能受到钉钉企业认证限制，表现为 `quotedFile.resolve` 返回 `orgAuthLevelNotEnough`。出现该错误时，群聊文件首次恢复将失败并降级为提示文本，但不会影响图片、图文、卡片、单聊文件等其他已确定性支持的引用场景。
