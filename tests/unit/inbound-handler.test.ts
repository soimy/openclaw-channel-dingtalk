...
describe("inbound-handler", () => {
  ...
  it("injects DingTalk media reply guidance without switching source delivery to message_tool_only", async () => {
    const runtime = buildRuntime();
    shared.getRuntimeMock.mockReturnValueOnce(runtime);
    ...
    await handleDingTalkMessage({
      ...
      dingtalkConfig: { dmPolicy: "open", messageType: "markdown", ackReaction: "" } as any,
      data: {
        msgId: "m_media_guidance",
        msgtype: "text",
        text: { content: "/tmp/输出 图片.png 把这张图发我" },
        conversationType: "1",
        conversationId: "cid_media_guidance",
        senderId: "user_1",
        chatbotUserId: "bot_1",
        sessionWebhook: "https://session.webhook",
        createAt: Date.now(),
    ...
    expect(finalized.GroupSystemPrompt).toContain("DingTalk media replies");
    expect(finalized.GroupSystemPrompt).toContain("MEDIA:<path>");
    expect(finalized.GroupSystemPrompt).toContain("/tmp/输出 图片.png");
    expect(finalized.GroupSystemPrompt).toContain("generated by an image tool");
