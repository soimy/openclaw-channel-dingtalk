...
describe("resolveGatewayCapabilityConfig", () => {
  it("defaults all capabilities to disabled when gatewayCapabilities is unset", () => {
    const caps = resolveGatewayCapabilityConfig(makeCfg({ clientId: "id" }));
    expect(caps).toEqual({ docsEnabled: false, proactiveSendEnabled: false });
  ...
  it("named accounts inherit channel-level gatewayCapabilities defaults", () => {
    const cfg = makeCfg({
      gatewayCapabilities: { tools: { docs: false, proactiveSend: true } },
...
describe("checkDocsGatewayCapability", () => {
  it("returns null when explicitly enabled and no allowlist", () => {
  ...
  it("denies when docs tool disabled", () => {
    ...
    expect(denial).toContain("gatewayCapabilities.tools.docs");
  });

  it("denies spaceId outside allowlist", () => {
    const caps = resolveGatewayCapabilityConfig(
      makeCfg({ gatewayCapabilities: { tools: { docs: true }, docs: { allowedSpaceIds: ["spaceA"] } } }),
    );
...
describe("checkProactiveSendGatewayCapability", () => {
  it("returns null when explicitly enabled and no allowlist", () => {
    const caps = resolveGatewayCapabilityConfig(makeCfg({ gatewayCapabilities: { tools: { proactiveSend: true } } }));
  ...
  it("denies when proactiveSend disabled", () => {
    ...
    expect(denial).toContain("gatewayCapabilities.tools.proactiveSend");
  });

  it("denies target outside allowlist", () => {
    const caps = resolveGatewayCapabilityConfig(
      makeCfg({ gatewayCapabilities: { tools: { proactiveSend: true }, send: { allowedTargets: ["group:g1"] } } }),
    );
...
describe("docs allowlist edge cases (review follow-up)", () => {
  ...
  it("denies disabled proactiveSend even for empty target", () => {
    ...
    expect(checkProactiveSendGatewayCapability(caps, "")).toContain("gatewayCapabilities.tools.proactiveSend");
  });
});
...
describe("allowlist fail-closed semantics (review follow-up)", () => {
  it("treats an empty allowedSpaceIds list as deny-all", () => {
    const caps = resolveGatewayCapabilityConfig(
      makeCfg({ gatewayCapabilities: { tools: { docs: true }, docs: { allowedSpaceIds: [] } } }),
    );
  ...
  it("treats an empty allowedTargets list as deny-all", () => {
    const caps = resolveGatewayCapabilityConfig(
      makeCfg({ gatewayCapabilities: { tools: { proactiveSend: true }, send: { allowedTargets: [] } } }),
    );
    expect(checkProactiveSendGatewayCapability(caps, "user:u1")).toContain("allowedTargets");
  ...
  it("unset allowlists stay unrestricted", () => {
    const caps = resolveGatewayCapabilityConfig(
      makeCfg({ gatewayCapabilities: { tools: { docs: true, proactiveSend: true } } }),
    );
