...
function buildDingTalkMediaReplyPrompt(): string {
  return [
    "DingTalk media replies:",
    "- To send a local image or file back to the user, put each attachment on its own line as MEDIA:<path>; keep any caption as normal reply text.",
    "- If a local image was generated by an image tool, send that generated image back with MEDIA:<path>.",
    "- Preserve spaces and non-ASCII characters in paths, for example MEDIA:/tmp/输出 图片.png or MEDIA:~/.openclaw/workspace/临时 输出/生成 图片.png.",
    "- Do not only describe the path as plain text when the user asks you to send the media.",
  ].join("\n");
}
...
export async function handleDingTalkMessage(params: HandleDingTalkMessageParams): Promise<void> {
  ...
  try {
    ...
    const extraSystemPrompt =
      [...groupSystemPromptParts, buildDingTalkMediaReplyPrompt(), learningContextBlock]
        .filter(Boolean)
        .join("\n\n") || undefined;
